...
`[step₁, step₂, …, stepₙ]`.
"""
→(prompt::AbstractPromptTemplate, model::AbstractLangModel) =
    SequentialFlow([prompt, model])
→(prompt::AbstractPromptTemplate, flow::SequentialFlow) =
    SequentialFlow([prompt; flow.steps])
→(flow::SequentialFlow, prompt::AbstractPromptTemplate) =
    SequentialFlow([flow.steps; prompt])
...
→(process::AbstractProcess, prompt::AbstractPromptTemplate) =
    SequentialFlow([process, prompt])
